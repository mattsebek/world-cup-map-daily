...
function BackdropGlobe(){
  ...
  useEffect(()=>{
    ...
    const draw=()=>{
      ...
      ctx.beginPath(); WORLD.features.forEach(f=>path(f));
      ctx.fillStyle="rgba(30,40,92,0.92)"; ctx.fill();
      ctx.strokeStyle="rgba(0,240,216,0.18)"; ctx.lineWidth=0.5; ctx.stroke();
      ctx.beginPath(); ctx.arc(SIZE/2,SIZE/2,SIZE/2-3,0,2*Math.PI);
      ctx.strokeStyle="rgba(0,240,216,0.22)"; ctx.lineWidth=1.4; ctx.stroke();
    };
    if(reduced){ draw(); return; }
    const tick=(now)=>{ const dt=now-last; last=now; lon+=dt*0.0035; if(lon>=360)lon-=360; draw(); raf=requestAnimationFrame(tick); };
